...
import AnimatedBackground from './components/background.jsx'; // Import our new component

const Home = () => {
    // Use React state to manage dynamic classes
    const [isMenuOpen, setMenuOpen] = useState(false);
    const [isScrolled, setScrolled] = useState(false);

    // This useEffect handles animations and event listeners
    useEffect(() => {
        // --- Event Listeners ---
        const handleScroll = () => {
        ...
        anime.timeline({easing: "easeOutExpo", duration: 1200})
            ...
            .add({
                targets: ".hero-content p, .hero-content a, .iso-phone-container",
                translateY: [30, 0],
                opacity: [0, 1],
        ...
        <div>
            ...
            <div id="content-wrapper">
                <header id="main-header"
                        className={`glass-nav fixed top-0 left-0 right-0 z-50 ${isScrolled ? 'scrolled' : ''}`}>
                    <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                        <Link to="/" className="flex items-center gap-3 anim-el">
                            <div
                                className="w-10 h-10 rounded-xl flex items-center justify-center bg-gradient-to-br from-secondary to-primary">
                        ...
                        <nav className="hidden md:flex items-center gap-8 text-sm font-semibold">
                            <a href="#reviews"
                               className="nav-link text-text-secondary anim-el flex items-center"><FileText
                                className="w-4 h-4 mr-2"/>Reviews</a>
                            <a href="#news" className="nav-link text-text-secondary anim-el flex items-center"><Rss
                                className="w-4 h-4 mr-2"/>News</a>
                            <a href="#compare"
                               className="nav-link text-text-secondary anim-el flex items-center"><GitPullRequest
                                className="w-4 h-4 mr-2"/>Compare</a>
                        </nav>
                        <div className="flex items-center gap-4">
                            <Link to="/login"
                                  className="cta-button px-5 py-2 rounded-lg text-sm font-semibold hidden md:block anim-el flex items-center"><LogIn
                                className="w-4 h-4 mr-2"/>Sign In</Link>
                            <button id="hamburger-btn"
                                    className={`md:hidden z-50 space-y-1.5 ${isMenuOpen ? 'open' : ''}`}
                ...
                <div id="mobile-menu"
                     className={`fixed top-0 right-0 h-full w-64 bg-surface/95 backdrop-blur-lg z-40 p-8 flex flex-col items-center justify-center ${isMenuOpen ? 'open' : ''}`}
                        ...
                        <div className="grid md:grid-cols-2 items-center gap-16">
                            <div className="hero-content z-10 text-center md:text-left">
                                ...
                                <p className="text-lg lg:text-xl text-text-secondary max-w-xl mb-10 mx-auto md:mx-0 anim-el">
                                    The definitive source for mobile technology. We cut through the noise to bring you
                                    reviews and analysis you can trust.
                                </p>
                                <a href="#reviews"
                                   className="cta-button px-8 py-4 rounded-xl font-semibold text-white inline-flex items-center anim-el">
